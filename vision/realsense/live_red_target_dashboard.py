...
<script>
const COORDINATE_DISPLAY_GRACE_MS=1500;const DISPLAY_WIDTH=1280, frame=document.getElementById('frame'), stage=document.getElementById('stage'), box=document.getElementById('box'), targetLabel=document.getElementById('target-label'), output=document.getElementById('state'), referenceOutput=document.getElementById('reference-state'), overlay=document.getElementById('reference-overlay'), confirmation=document.getElementById('registration-confirmation'), registerButton=document.getElementById('register-board'), coordinateCard=document.getElementById('target-coordinate'), coordinateX=document.getElementById('target-x'), coordinateY=document.getElementById('target-y'), coordinateState=document.getElementById('target-coordinate-state');let lastValidCoordinate=null;
...
function renderTargetCoordinate(state){
  ...
  if(hasCoordinate){lastValidCoordinate={x:Number(coordinate.x),y:Number(coordinate.y),observedAtMs:Date.now()};coordinateX.textContent=formatCoordinate(coordinate.x);coordinateY.textContent=formatCoordinate(coordinate.y);coordinateState.textContent='实时红方块坐标';coordinateCard.dataset.state='ready';return;}
  if(lastValidCoordinate&&state.target&&Date.now()-lastValidCoordinate.observedAtMs<=COORDINATE_DISPLAY_GRACE_MS){coordinateX.textContent=formatCoordinate(lastValidCoordinate.x);coordinateY.textContent=formatCoordinate(lastValidCoordinate.y);coordinateState.textContent='等待有效映射，显示上次有效坐标';coordinateCard.dataset.state='stale';return;}
  lastValidCoordinate=null;coordinateX.textContent='--';coordinateY.textContent='--';coordinateState.textContent=state.state==='no_target'?'未检测到有效红方块':'坐标不可用';coordinateCard.dataset.state='unavailable';
}
